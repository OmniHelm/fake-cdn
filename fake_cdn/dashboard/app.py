...
def _build_where(tenant_id, start_time=None, end_time=None, domain=None, project=None):
    """构造租户内 SQL 条件；tenant_id 缺失时拒绝查询，避免跨租户泄漏。"""
    if not tenant_id:
        raise ValueError("tenant_id 是数据查询的必填隔离条件")
    where = ["tenant_id = ?", "interval > 0"]
...
def _load_aggregate_meta(
    storage, tenant_id, start_time=None, end_time=None, domain=None, project=None
):
    """单条 SQL 返回时段+过滤条件下的全部聚合指标，供概览与分析页共享"""
    where_sql, params = _build_where(tenant_id, start_time, end_time, domain, project)
...
def _load_peak_bps(storage, tenant_id, start_time=None, end_time=None, domain=None, project=None):
...
def load_analytics_data(
    ...
    meta = _load_aggregate_meta(storage, tenant_id, start_time, end_time, domain, project)
...
def _resolve_end_time_ms(storage, tenant_id):
...
def get_default_date_range(storage: CDNLogStorage, tenant_id: str):
    """获取默认日期范围：以统一基准时间向前推 7 天，对齐概览默认"周" """
    min_time, max_time = storage.get_time_range(tenant_id=tenant_id)
...
def create_metric_card(title, value, subtitle=None, color=None, trend=None):
    ...
    return html.Div(
        [
            ...
            html.Div(
                value_children, className="metric-value", style={"color": color} if color else {}
            ),
...
def create_summary_cards(df):
    ...
    daily_stats = time_agg.groupby("date").agg({"bw_mbps": ["mean", calc_95_billing]})
    ...
    return html.Div(
        [
            create_metric_card(
                "峰值带宽",
                f"{peak_bw/1000:.1f} Gbps",
                f"平均 {avg_bw/1000:.1f} Gbps",
                trend=bw_trend,
            ),
            create_metric_card("日平均带宽", f"{daily_avg_bw/1000:.1f} Gbps", "每日平均值"),
            create_metric_card(
                "日95带宽", f"{daily_p95_bw/1000:.1f} Gbps", "95th 分位计费带宽", COLORS["primary"]
            ),
...
def create_topbar(
    ...
    if is_admin:
        tenant_control = html.Div(
            [
                ...
                dcc.Dropdown(
                    id="global-tenant-switcher",
                    options=[
                        {
                            "label": item.get("display_name") or item["tenant_id"],
    ...
    if auth_config["enabled"]:
        right_items.append(html.A("退出登录", href="/logout", className="topbar-logout"))
    ...
    return html.Div(
        [
            ...
            html.Div(
                [
                    html.Button(
                        "最近 24 小时", id="range-24h", className="time-range-btn", n_clicks=0
                    ),
                    html.Button(
                        "最近 7 天", id="range-7d", className="time-range-btn active", n_clicks=0
                    ),
                    html.Button(
                        "最近 30 天", id="range-30d", className="time-range-btn", n_clicks=0
                    ),
                    html.Button(
                        "自定义范围", id="range-custom", className="time-range-btn", n_clicks=0
                    ),
...
def _compute_overview_metrics(storage, tenant_id, range_key="week"):
    ...
    peak_bps = _load_peak_bps(storage, tenant_id, start_time=start_ms, end_time=end_ms)
...
def _compute_activity_timeline(
    ...
    try:
        with storage._get_conn() as conn:
            ...
            peak_row = conn.execute(
                ...
                "WHERE tenant_id = ? AND interval > 0 AND start_time >= ? AND start_time <= ? "
    ...
    if domain_count:
        activities.append(
            ("近 24h", "info", f"活跃域名 {domain_count} 个 · 总请求 {total_req/1e6:.2f} M")
        )
    ...
    if max_time:
        ...
        activities.append(
            (_humanize_relative(now_dt, max_time), "success", f"最新数据到达 {ts_str}")
        )
...
def create_overview_page(storage, tenant_id):
    """概览页 (带 日/周/月 筛选 + SQL 层实时聚合)"""
    default_range = "week"
    metrics = _compute_overview_metrics(storage, tenant_id, default_range)
    ...
    return html.Div(
        [
            create_page_header("概览", f"租户 {tenant_id} 的 CDN 运行状态与关键指标"),
            range_bar,
...
def _build_domain_rows(pairs):
    ...
    for p in pairs:
        ...
        origin_ip = (
            f"{r.randint(10, 200)}.{r.randint(0, 255)}.{r.randint(0, 255)}.{r.randint(1, 254)}"
        )
    ...
    rows.extend(
        [
            ...
            },
            {
                "type": "CNAME",
                "name": "cdn",
                "value": f"{base_domain}.cdn.fakecdn.net",
                "ttl": "自动",
                "proxy": "已代理",
            },
            {
                "type": "CNAME",
                "name": "api",
                "value": f"api-origin.{base_domain}",
                "ttl": "300",
                "proxy": "仅 DNS",
            },
            {
            ...
            },
            {
                "type": "TXT",
                "name": "_dmarc",
                "value": "v=DMARC1; p=reject; rua=mailto:dmarc@...",
                "ttl": "3600",
                "proxy": "仅 DNS",
            },
            {
        ...
        rows.append(
            {
                "type": "A",
                "name": d,
                "value": f"203.0.113.{random.randint(1, 250)}",
                "ttl": "自动",
                "proxy": "已代理",
            }
        )
...
def create_cache_page():
    ...
    purge_card = html.Div(
        [
            html.H3("URL 刷新"),
            html.P(
                "输入 URL 立即清除 CDN 节点缓存，支持通配符 *",
                style={"fontSize": "13px", "color": "#64748b", "margin": "0 0 12px"},
            ),
            html.Div(
                [
                    dcc.Input(
    ...
    prefetch = [
        {
            "url": "https://appcircle.io/static/bundle.js",
            "status": "已完成",
            "progress": "100%",
            "time": "12:30",
        },
        {
            "url": "https://tigerbeetle.com/images/logo.png",
            "status": "进行中",
            "progress": "64%",
            "time": "12:35",
        },
...
def create_rules_page():
    ...
    rules = [
        {
            "priority": "1",
            "pattern": "*/admin/*",
            "action": "强制 HTTPS + 缓存级别: 绕过",
            "status": "启用",
        },
        {
            "priority": "2",
            "pattern": "*/api/v2/*",
            "action": "禁用缓存 + 请求头: X-API-Version",
            "status": "启用",
        },
        {
            "priority": "3",
            "pattern": "*/static/*",
            "action": "边缘缓存 TTL: 30 天",
            "status": "启用",
        },
        {
            "priority": "4",
            "pattern": "*.mp4|*.webm",
            "action": "边缘缓存 TTL: 7 天 + 启用 Range",
            "status": "启用",
        },
    ...
    return html.Div(
        [
            create_page_header(
                "流量规则", "基于 URL 模式的路由与缓存规则", action_label="+ 新建规则"
            ),
            html.Div(
                [
                    html.Div(
                        "规则按优先级从小到大匹配，命中后不再继续。",
...
def create_loadbalancer_page():
    ...
    pools = [
        ...
        {
            "name": "secondary-pool-cn",
            "healthy": "3 / 4",
            "algorithm": "最少连接",
            "weight": "100,100,100,0",
            "status": "部分降级",
        },
        {
            "name": "global-pool",
...
# 页面: 月度报告
# ============================================================================
def _list_available_months(storage, tenant_id):
...
def _build_report_content(storage, tenant_id, month_str, project):
    """构造月度报告主体（供初始化与回调复用）"""
    if not month_str:
        return html.Div(
            "暂无数据", style={"color": "#94a3b8", "padding": "40px", "textAlign": "center"}
        )

    start_ms, end_ms = _month_bounds(month_str)
    proj = project if project and project != "all" else None

    meta = _load_aggregate_meta(storage, tenant_id, start_ms, end_ms, project=proj)
    peak_bps = _load_peak_bps(storage, tenant_id, start_ms, end_ms, project=proj)
    daily = _load_daily_stats(storage, tenant_id, start_ms, end_ms, project=proj)
    ...
    summary = html.Div(
        [
            ...
            create_metric_card(
                "峰值带宽", f"{peak_gbps:.2f} Gbps", "月内瞬时峰值", COLORS["primary"]
            ),
            ...
            create_metric_card(
                "可用率",
                f"{avail:.2f}%" if avail is not None else "—",
                "回源成功率",
    ...
    # 域名 Top 10
    with storage._get_conn() as conn:
        where_sql, params = _build_where(tenant_id, start_ms, end_ms, None, proj)
    ...
    if not top_df.empty:
        ...
        top_df["hit_rate"] = (top_df["hit"] / top_df["req"].where(top_df["req"] > 0) * 100).fillna(
            0
        )
    ...
    report_title = html.Div(
        [
            html.Div(
                f"{month_str} 月度 CDN 流量报告",
                style={"fontSize": "24px", "fontWeight": "700", "color": "#0f172a"},
            ),
            html.Div(
                f"项目: {project_label} · 涵盖 {len(daily)} 天 · {domain_count} 个域名 · "
            ...
            html.Div(
                [
                    html.Div(
                        [
                            (
                                dcc.Graph(figure=top_fig, config={"displayModeBar": False})
                                if top_fig
                                else html.Div("无数据")
                            )
                        ],
...
def create_report_page(storage, tenant_id, projects):
    """月度报告页面"""
    months = _list_available_months(storage, tenant_id)
    ...
    initial = _build_report_content(storage, tenant_id, default_month, "all")
...
def create_app(data_file=None, config_path=None, config_db_path=None):
    """创建 Dash 应用"""
    # 获取 SQLite 存储
    storage = get_storage()
    resolved_config_path = (
        Path(config_path or os.environ.get("FAKE_CDN_CONFIG_PATH") or Path.cwd() / "config.json")
        .expanduser()
        .resolve()
    )
    config_manager = (
        TenantConfigStore(config_db_path) if config_db_path else get_default_config_store()
    )
    # 主配置始终可导入；其他历史配置仅在日志库中存在对应租户时导入。
    config_manager.bootstrap([resolved_config_path])
    log_tenants = storage.get_tenants()
    additional_configs = [
        path
        for path in sorted(resolved_config_path.parent.glob("config*.json"))
        if path.resolve() != resolved_config_path
    ]
    config_manager.bootstrap(additional_configs, allowed_tenant_ids=log_tenants)
    ...
    default_tenant_id = next(
        ...
        tenant_rows[0]["tenant_id"] if tenant_rows else None,
    )
    if not default_tenant_id:
        raise RuntimeError("配置数据库中没有租户，请先运行 tenant-migrate")
    ...
    app.server.secret_key = os.environ.get(
        "DASHBOARD_SECRET_KEY", secrets.token_hex(32)  # 如果未设置则生成随机 key（重启后失效）
    )
    ...
    def login():
        ...
        return LOGIN_PAGE.format(
            error_message=error_message, error_display=error_display, username=username
        )
    ...
    def update_all(
        ...
        try:
            ...
            traceback.print_exc()
            return ("数据加载失败", html.Div(), f"错误: {e}", {}, {}, {}, {}, {}, {}, {}, {}, [])

        if not data["time_rows"]:
            return ("暂无数据", html.Div(), "无数据", {}, {}, {}, {}, {}, {}, {}, {}, [])
        ...
        project_label = (
            selected_project if selected_project and selected_project != "all" else "全部项目"
        )
        header_info = (
        ...
        if data["domains"]:
            ...
            domain_df["flux_gb"] = pd.to_numeric(domain_df["flux"], errors="coerce").fillna(0) / (
                1024**3
            )
        ...
        origin_fig.update_yaxes(
            title_text="回源带宽 (Mbps)", secondary_y=False, title_font={"size": 11}
        )
        ...
        for r in data["table"]:
            ...
            table_rows.append(
                {
                    "timestamp": datetime.fromtimestamp((r["start_time"] or 0) / 1000).strftime(
                        "%m-%d %H:%M:%S"
                    ),
                    "domain": r.get("domain"),
    ...
    @app.callback(
        Output("url", "pathname"),
        Input("global-tenant-switcher", "value"),
        [State("url", "pathname"), State("tenant-context", "data")],
        prevent_initial_call=True,
    )
    def switch_tenant(tenant_id, pathname, current_tenant_id):
        ...
        if len(parts) >= 3 and parts[0] == "tenants":
            return f"/tenants/{tenant_id}/{parts[2]}"
    ...
    def render_page(pathname):
        ...
        if not admin_access and pathname.startswith(("/config", "/admin")):
            ...
            if len(parts) >= 3 and parts[0] == "config" and parts[1] == "tenants":
                ...
                elif len(parts) > 3 and parts[3] == "audit":
                    page = create_config_audit_page(config_manager, tenant_id)
                else:
                    page = html.Div(
                        [
                            create_config_page(config_manager, tenant_id),
                            create_version_panel(config_manager, tenant_id),
                        ]
                    )
            else:
                if len(parts) >= 3 and parts[0] == "tenants":
...
def run_dashboard(
    ...
    if auth_config["enabled"]:
        ...
        print("  登录认证: 未启用 (设置 DASHBOARD_PASSWORD 环境变量启用)")
    print("=" * 60)
